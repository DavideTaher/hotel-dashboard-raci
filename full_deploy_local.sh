...
echo "🚀 DEPLOY COMPLETO LOCAL - HOTEL DASHBOARD RACI"
echo "==============================================="
...
FRONTEND_DIR="$HOTEL_DIR/frontend_local"

cd "$HOTEL_DIR"

echo "✅ Configurando sistema completo local..."

# Crear estructura frontend local
mkdir -p "$FRONTEND_DIR"/{src,public,dist}

# Crear package.json para frontend local
cat > "$FRONTEND_DIR/package.json" << 'PACKAGE'
{
  "name": "hotel-dashboard-raci",
  "version": "1.0.0",
  "description": "Hotel Dashboard RACI - Sistema Local",
  "main": "index.html",
  "scripts": {
    "dev": "python3 -m http.server 8080",
    "build": "echo 'Build local ready'",
    "start": "open http://localhost:8080"
  }
}
PACKAGE

# Crear dashboard React-style con vanilla JS
cat > "$FRONTEND_DIR/index.html" << 'FRONTEND'
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
...
# Crear servidor local simple
cat > "$FRONTEND_DIR/start_server.py" << 'SERVER'
#!/usr/bin/env python3
import http.server
import socketserver
import webbrowser
...
# Script para iniciar dashboard local
cat > "$HOTEL_DIR/start_dashboard_local.sh" << 'START_SCRIPT'
#!/bin/bash
echo "🚀 INICIANDO HOTEL DASHBOARD RACI - LOCAL"
echo "========================================"
cd ~/hotel_dashboard_raci/frontend_local
python3 start_server.py
START_SCRIPT

chmod +x "$HOTEL_DIR/start_dashboard_local.sh"

# Agregar aliases para dashboard local
ALIASES_LOCAL=(
    'alias dashboard_local="~/hotel_dashboard_raci/start_dashboard_local.sh"'
    'alias open_banks="open ~/hotel_dashboard_raci/daily_exports/bac_colones/incoming/ && open ~/hotel_dashboard_raci/daily_exports/bcr_colones/incoming/"'
    'alias process_and_view="~/hotel_dashboard_raci/daily_exports/process_daily.sh && sleep 2 && open http://localhost:8080"'
)

for alias_cmd in "${ALIASES_LOCAL[@]}"; do
...
echo "   dashboard_local     - Iniciar dashboard en navegador"
echo "   open_banks         - Abrir carpetas de bancos para subir archivos"
echo "   process_and_view   - Procesar datos y abrir dashboard"
echo "   exports_daily      - Procesar exports solamente"
echo ""
echo "📊 DASHBOARD LOCAL:"
echo "   URL: http://localhost:8080"
echo "   Archivo: ~/hotel_dashboard_raci/frontend_local/index.html"
echo ""
echo "🏦 PARA AGREGAR ARCHIVOS DE BANCO:"
echo "   1. Ejecutar: open_banks"
echo "   2. Copiar archivos .csv/.xlsx a las carpetas"
echo "   3. Ejecutar: process_and_view"
echo ""
echo "✨ LISTO PARA PROBAR CON DATOS REALES"
